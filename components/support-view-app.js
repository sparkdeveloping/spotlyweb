...
export function SupportViewApp({ businessId }) {
  ...
  return <AuthGate portal="admin" title="Sign in to Spotly Admin"><main className="min-h-screen bg-grouped text-ink">
    <header className="border-b bg-white"><div className="mx-auto flex min-h-[72px] max-w-7xl items-center justify-between gap-4 px-4 sm:px-6"><Link href="/admin/businesses" className="inline-flex items-center gap-2 text-sm font-bold text-admin"><ArrowLeft className="h-4 w-4" />Back to businesses</Link><Badge tone="warning"><ShieldCheck className="h-3.5 w-3.5" />Audited administrator view</Badge></div></header>
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      {!allowed ? <Card elevated className="mx-auto max-w-xl p-8"><EmptyState icon={ShieldCheck} title="This role cannot open support view" description="A super administrator can grant a support or operations role from People & access." /></Card> : !context ? <Card elevated className="mx-auto max-w-2xl p-7"><span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-admin-soft text-admin"><Building2 className="h-7 w-7" /></span><h1 className="mt-6 text-3xl font-black tracking-[-.04em]">Open business support view</h1><p className="mt-3 leading-7 text-secondary">This provides a read-only operational snapshot without impersonating the owner. The business, administrator, reason, and timestamp are written to the audit log.</p><div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950"><div className="flex gap-3"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0" /><p>Use only for a legitimate support, verification, or operational task. Do not copy sensitive information outside approved Spotly systems.</p></div></div><form onSubmit={openView} className="mt-6"><label className="block"><span className="mb-2 block text-sm font-bold">Reason for access</span><textarea required minLength={8} maxLength={500} value={reason} onChange={(event) => setReason(event.target.value)} className="surface min-h-28 w-full rounded-2xl p-4 outline-none focus:ring-2 focus:ring-admin/20" placeholder="Example: Investigating branch catalogue setup reported in support conversation…" /></label><Button type="submit" loading={loading} className="mt-4 w-full">Open audited support view</Button></form></Card> : <div className="space-y-6">
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-950"><div className="flex flex-col gap-3 sm:flex-row sm:items-center"><ShieldCheck className="h-5 w-5 shrink-0" /><div className="min-w-0 flex-1"><p className="font-bold">Support view active — read only</p><p className="mt-1 text-sm">Reason: {context.session.reason}</p></div><div className="flex items-center gap-2 text-xs font-bold"><Clock3 className="h-4 w-4" />Expires {new Date(context.session.expiresAt).toLocaleTimeString()}</div></div></div>
        <div><p className="text-xs font-bold uppercase tracking-[.16em] text-admin">Business support context</p><h1 className="mt-2 text-4xl font-black tracking-[-.045em]">{context.business.name}</h1><p className="mt-2 text-secondary">{context.business.category} · {context.business.city || "Zimbabwe"} · {user?.email}</p></div>
