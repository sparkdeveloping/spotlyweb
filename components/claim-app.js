...
import { Button, Card, ProgressBar, SearchField, Badge } from "@/components/ui";
...
export function ClaimApp({ initialBusinessId, newBusiness = false, initialName = "" }) {
  ...
  async function submit() {
    ...
    try {
      ...
      setTask({ open: true, state: "success", active: 4, title: result.autoApproved ? "Your workspace is ready" : "Claim submitted successfully", description: result.autoApproved ? "The configured low-risk policy approved this claim. Continue with guided business setup." : "Spotly saved the claim and selected locations. Any request for more information will appear in your business workspace." });
    } catch (error) {
