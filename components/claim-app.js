...
import { deleteClaimDraft, getBranchesForBusiness, getBusiness, getClaimDraft, saveClaimDraft, searchBusinesses, track, uploadClaimEvidence } from "@/lib/firebase-services";
...
function BusinessSearch({ onSelect, initialName = "" }) {
  ...
  useEffect(() => { const timer=window.setTimeout(async()=>{const term=query.trim();if(term.length<2){setResults([]);setError("");return;}setLoading(true);setError("");try{setResults(await searchBusinesses(term,20));}catch{setResults([]);setError("Business search is temporarily unavailable.");}finally{setLoading(false);}},250);return()=>window.clearTimeout(timer);},[query]);
  return <div><SearchField value={query} onChange={setQuery} placeholder="Business or brand name" className="h-14" />{query.length>=2&&<div className="mt-4 overflow-hidden rounded-xl border bg-white">{loading?<div className="p-5 text-sm text-secondary">Searching businesses…</div>:error?<div role="alert" className="p-5 text-sm text-danger">{error}</div>:results.length?results.map((business)=><button key={business.id} type="button" onClick={()=>onSelect(business)} className="flex w-full items-center gap-3 border-b px-4 py-4 text-left last:border-0 hover:bg-violet-soft/50"><Store className="h-5 w-5 text-violet" /><span className="min-w-0 flex-1"><span className="block truncate font-semibold">{business.brandName||business.name}</span><span className="mt-1 block truncate text-sm text-secondary">{[business.category,business.city].filter(Boolean).join(" · ")}</span></span><ChevronRight className="h-5 w-5 text-tertiary" /></button>):<div className="p-5"><p className="font-semibold">No matching business was found.</p><p className="mt-2 text-sm text-secondary">Create the business and first location as part of this request.</p><Button type="button" className="mt-4" onClick={()=>onSelect({id:null,name:query,isNew:true})}>Add “{query}”</Button></div>}</div>}</div>;
}

function Choice({ active, onClick, title, description, icon: Icon }) {
  return <button type="button" onClick={onClick} className={`rounded-xl border p-4 text-left ${active?"border-violet bg-violet-soft ring-2 ring-violet/10":"bg-white hover:border-violet/30"}`}>{Icon&&<Icon className="h-5 w-5 text-violet" />}<p className={Icon?"mt-3 font-semibold":"font-semibold"}>{title}</p><p className="mt-1 text-sm leading-6 text-secondary">{description}</p></button>;
}
...
export function ClaimApp({ initialBusinessId, newBusiness = false, initialName = "", initialDraftId = "" }) {
  ...
  useEffect(()=>{if(!ready||submitting)return;const timer=window.setTimeout(async()=>{const data=snapshot();writeState(DRAFT_KEY,user,data);setLastSaved(data.savedAt);if(user?.uid){try{const id=await saveClaimDraft(user.uid,data);setDraftId(id);}catch{}}},500);return()=>window.clearTimeout(timer);},[ready,stepIndex,mode,selected,branches,selectedBranchIds,form,evidence,user?.uid,submitting]);
  ...
  async function saveAndExit(){setSaving(true);try{const data=snapshot();writeState(DRAFT_KEY,user,data);if(user?.uid){const id=await saveClaimDraft(user.uid,data);setDraftId(id);router.push("/claim/drafts");}else router.push(`/login?portal=business&next=${encodeURIComponent("/claim")}`);}catch(saveError){setError(saveError.message);}finally{setSaving(false);}}
  ...
  const stepContent = {
    ...
    relationship:<div className="space-y-6"><div><h2 className="text-2xl font-semibold">Confirm your relationship</h2><p className="mt-2 text-sm leading-6 text-secondary">This helps Spotly request the right evidence and access scope.</p></div><div className="grid gap-3 sm:grid-cols-2">{[["owner","Owner or director","Request brand or organization ownership."],["franchisee","Franchisee","Request access to authorized franchise locations."],["branch_manager","Location manager","Request operational access to one location."],["authorized_staff","Authorized staff","Request limited access approved by the business."]].map(([value,title,description])=><Choice key={value} active={form.roleAtBusiness===value} onClick={()=>setForm({...form,roleAtBusiness:value})} title={title} description={description} />)}</div><div className="grid gap-4 sm:grid-cols-2"><Field label="Your name"><input className="input w-full" value={form.applicantName} onChange={(event)=>setForm({...form,applicantName:event.target.value})} /></Field><Field label="Phone"><input className="input w-full" value={form.phone} onChange={(event)=>setForm({...form,phone:event.target.value})} placeholder="+263" /></Field><Field label="Business email" className="sm:col-span-2"><input type="email" className="input w-full" value={form.email} onChange={(event)=>setForm({...form,email:event.target.value})} /></Field></div></div>,
    parent:<div className="space-y-6"><div><h2 className="text-2xl font-semibold">Confirm the parent organization</h2><p className="mt-2 text-sm leading-6 text-secondary">Tell us whether head office or another company controls the brand.</p></div><div className="grid gap-3 sm:grid-cols-3">{[["none","No parent company","The business is independently controlled."],["existing","A parent company exists","Head office or another legal entity governs the brand."],["unknown","I am not sure","Spotly should investigate the relationship."]].map(([value,title,description])=><Choice key={value} active={form.parentMode===value} onClick={()=>setForm({...form,parentMode:value})} title={title} description={description} />)}</div>{form.parentMode!=="none"&&<div className="grid gap-4 sm:grid-cols-2"><Field label="Parent organization name"><input className="input w-full" value={form.parentName} onChange={(event)=>setForm({...form,parentName:event.target.value})} /></Field><Field label="Head-office contact email"><input type="email" className="input w-full" value={form.parentContactEmail} onChange={(event)=>setForm({...form,parentContactEmail:event.target.value})} /></Field><Field label="Head-office contact name" className="sm:col-span-2"><input className="input w-full" value={form.parentContactName} onChange={(event)=>setForm({...form,parentContactName:event.target.value})} /></Field></div>}<div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-800">If another verified owner controls the business, Spotly will open an ownership review rather than replacing access silently.</div></div>,
  ...
  return <main className="min-h-screen bg-grouped px-4 py-8 sm:px-6"><div className="mx-auto max-w-6xl"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><Link href="/" className="text-sm font-semibold text-violet">← Spotly home</Link><h1 className="mt-3 text-3xl font-semibold">Claim or add a business</h1><p className="mt-2 text-sm text-secondary">Step {stepIndex+1} of {STEPS.length}: {step.label}</p></div><div className="flex items-center gap-3"><span className="text-xs text-secondary">{lastSaved?`Saved ${new Date(lastSaved).toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})}`:"Not saved yet"}</span><Button variant="outline" loading={saving} onClick={saveAndExit}>Save and exit</Button></div></div><ProgressBar value={progress} className="mt-5" /><div className="mt-6 grid gap-6 lg:grid-cols-[230px_1fr]"><aside className="hidden lg:block"><ol className="space-y-1">{STEPS.map((item,index)=><li key={item.id}><button type="button" onClick={()=>index<stepIndex&&setStepIndex(index)} disabled={index>stepIndex} className={`flex w-full items-center gap-3 rounded-xl p-3 text-left text-sm ${index===stepIndex?"bg-violet-soft text-violet-strong":index<stepIndex?"hover:bg-white":"text-tertiary"}`}><span className={`flex h-8 w-8 items-center justify-center rounded-lg ${index<stepIndex?"bg-success text-white":index===stepIndex?"bg-violet text-white":"bg-white"}`}>{index<stepIndex?<Check className="h-4 w-4" />:index+1}</span><span className="font-medium">{item.label}</span></button></li>)}</ol><Button asChild variant="ghost" className="mt-4 w-full"><Link href="/claim/drafts">View saved drafts</Link></Button></aside><Card variant="bordered" className="overflow-hidden"><div className="p-5 sm:p-8">{error&&<div role="alert" className="mb-5 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div>}{stepContent}</div><div className="sticky bottom-0 flex items-center justify-between gap-3 border-t bg-white p-5"><Button variant="ghost" disabled={stepIndex===0} onClick={()=>{setError("");setStepIndex((value)=>Math.max(0,value-1));}}><ArrowLeft className="h-4 w-4" />Back</Button>{stepIndex<STEPS.length-1?<Button onClick={next}>Continue<ArrowRight className="h-4 w-4" /></Button>:null}</div></Card></div></div></main>;
}
