...
function ChatPanel({ onClose, initialTopic = "" }) {
  const { user, ensureAnonymousSession } = useAuth();
  const { settings } = usePlatform();
  const { toast } = useToast();
  const [conversationId, setConversationId] = useState("");
  const [messages, setMessages] = useState([]);
  const initialCategory = ["business_claim", "onboarding", "account", "orders", "payments", "partnership", "safety"].includes(initialTopic) ? initialTopic : "general";
  const initialSubject = initialTopic ? initialTopic.replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase()) : "";
  ...
  useEffect(() => {
    if (!initialTopic || conversationId) return;
    const category = ["business_claim", "onboarding", "account", "orders", "payments", "partnership", "safety"].includes(initialTopic) ? initialTopic : "general";
    const subject = initialTopic.replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase());
    setForm((current) => ({ ...current, category, subject: current.subject || subject }));
  }, [initialTopic, conversationId]);
  useEffect(() => {
