...
function InviteModal({ open, onClose }) {
  const { business, branches, user } = useBusinessWorkspace();
  ...
  return <Modal open={open} onClose={onClose} title="Invite a team member" size="lg"><form onSubmit={submit} className="space-y-5 p-5">
    ...
    <div><p className="text-sm font-semibold">Assigned branches</p><p className="mt-1 text-xs text-secondary">Leave every branch unchecked to allow business-wide access where the permission permits it.</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{branches.map((branch) => <label key={branch.id} className="flex items-center gap-3 rounded-xl border p-3"><input type="checkbox" checked={form.branchIds.includes(branch.id)} onChange={() => toggleBranch(branch.id)} /><span className="text-sm font-medium">{branch.name}</span></label>)}</div></div>
...
export function StaffView() {
  ...
  return <div className="space-y-6">
    <PageHeader title="Staff & access" description="Give each person only the branches and controls they need." actions={<><BusinessSwitcher /><Button variant="outline" loading={busy} onClick={reviewTeam}><ShieldCheck className="h-4 w-4" />Confirm access review</Button><Button onClick={() => setInviteOpen(true)}><Plus className="h-4 w-4" />Invite teammate</Button></>} />
    ...
    {tab === "team" ? <SectionCard title="Active access" description="Owners and staff with current access to this business.">{visibleMembers.length ? <div className="divide-y">{visibleMembers.map((member) => <div key={member.id} className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-business-soft text-business"><UserRound className="h-5 w-5" /></span><div className="min-w-0 flex-1"><p className="truncate font-semibold">{member.displayName || member.name || member.email || member.userId}</p><p className="mt-1 truncate text-xs text-secondary">{member.email || member.userId} · {businessRoleTemplates.find((item) => item.id === member.role)?.name || member.role}</p><div className="mt-2 flex flex-wrap gap-1">{member.branchIds?.length ? member.branchIds.map((id) => <Badge key={id}>{branches.find((branch) => branch.id === id)?.name || "Assigned branch"}</Badge>) : <Badge tone="accent">Business-wide</Badge>}</div></div><StatusBadge status={member.status || "active"} /><Button size="sm" variant="outline" onClick={() => setEditing(member)}><MoreHorizontal className="h-4 w-4" />Manage</Button></div>)}</div> : <EmptyState icon={UsersRound} title="No active team records" description="Invite the first teammate or verify that your own membership was created correctly." action={<Button onClick={() => setInviteOpen(true)}><Plus className="h-4 w-4" />Invite teammate</Button>} />}</SectionCard> : <SectionCard title="Pending and previous invitations" description="Every invitation stays visible with its delivery and acceptance state.">{visibleInvites.length ? <div className="divide-y">{visibleInvites.map((invite) => <div key={invite.id} className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-grouped text-secondary"><Mail className="h-5 w-5" /></span><div className="min-w-0 flex-1"><p className="font-semibold">{invite.name || invite.email}</p><p className="mt-1 text-xs text-secondary">{invite.email} · {businessRoleTemplates.find((item) => item.id === invite.role)?.name || invite.role}</p></div><StatusBadge status={invite.status || "pending"} />{invite.status === "pending" && <div className="flex gap-2"><Button size="sm" variant="outline" disabled={busy} onClick={() => resend(invite)}><RefreshCw className="h-4 w-4" />Resend</Button><Button size="sm" variant="ghost" className="text-danger" onClick={() => setConfirm(invite)}><XCircle className="h-4 w-4" />Revoke</Button></div>}</div>)}</div> : <EmptyState icon={Mail} title="No invitations in this view" description="New invitations remain here until accepted or revoked." />}</SectionCard>}
    <Card className="p-5"><div className="flex items-start gap-4"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-emerald-50 text-success"><CheckCircle2 className="h-5 w-5" /></span><div><h2 className="font-bold">Access is explicit and reviewable</h2><p className="mt-1 text-sm leading-6 text-secondary">Branch access, roles, permissions, invitations, and account status are recorded clearly. Every change is included in the activity history.</p></div></div></Card>
    <InviteModal open={inviteOpen} onClose={() => setInviteOpen(false)} />
    <MemberModal member={editing} open={Boolean(editing)} onClose={() => setEditing(null)} />
