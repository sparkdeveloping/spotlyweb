...
function MemberModal({ member, open, onClose }) {
  ...
  async function submit(event) {
    ...
    try { await updateBusinessMembership(member.id, form, user, business.id); toast("Team access has been updated."); onClose(); }
...
export function StaffView() {
  const { business, members, invitations, branches, user, membership, selectedBusinessId } = useBusinessWorkspace();
  ...
  async function resend(invitation) {
    setBusy(true);
    try {
      await resendBusinessInvitation(invitation.id, user, business.id);
  ...
  async function revoke() {
    if (!confirm) return;
    setBusy(true);
    try { await revokeBusinessInvitation(confirm.id, user, business.id); toast("The invitation has been revoked."); setConfirm(null); }
