...
export function BusinessDashboard() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-5 border-b pb-6 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-sm font-semibold text-business">{data.business?.brandName || data.business?.name || "Spotly Business"}</p><h1 className="mt-2 text-3xl font-semibold tracking-[-.03em]">Today at {locationName}</h1><p className="mt-2 text-sm text-secondary">{locationCity} · {data.archetype.shortLabel}</p></div><div className="flex flex-col gap-3 sm:flex-row sm:items-center"><WorkspaceContextSwitcher /><Button asChild variant="outline"><Link href={href("/business/branches")}>Manage location</Link></Button></div></div>
    <CompletionBanner />
