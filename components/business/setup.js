...
function ReviewStep({ draft, branchDraft, selectedTemplateIds }) {
  ...
  return <div className="space-y-6"><div><p className="text-xs font-black uppercase tracking-[.16em] text-business">Final step</p><h2 className="mt-2 text-3xl font-black tracking-tight">A focused workspace is ready to be created</h2><p className="mt-3 max-w-2xl text-sm leading-7 text-secondary">Review the essentials. Detailed finance, staff, and publication settings will appear later—when they are useful.</p></div><div className="grid gap-4 md:grid-cols-2"><Card className="p-5"><p className="text-xs font-black uppercase tracking-[.14em] text-tertiary">Business</p><h3 className="mt-3 text-xl font-black">{draft.name}</h3><p className="mt-1 text-sm text-secondary">{archetype.label} · {draft.category}</p><p className="mt-4 text-sm leading-6 text-secondary">{draft.description || "Description can be completed from the guided setup centre."}</p></Card><Card className="p-5"><p className="text-xs font-black uppercase tracking-[.14em] text-tertiary">First {archetype.nouns.branch}</p><h3 className="mt-3 text-xl font-black">{branchDraft.branchName || branchDraft.name || "Main location"}</h3><p className="mt-1 text-sm text-secondary">{branchDraft.city || "Zimbabwe"}</p><p className="mt-4 text-sm leading-6 text-secondary">{branchDraft.address || "Address will be confirmed before publication."}</p></Card></div><Card className="p-5"><div className="flex items-start gap-4"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-business-soft text-business"><CheckCircle2 className="h-5 w-5" /></span><div><h3 className="font-bold">Spotly will now prepare</h3><ul className="mt-2 space-y-2 text-sm leading-6 text-secondary"><li>• A workspace shaped around {archetype.shortLabel.toLowerCase()} operations</li><li>• Clear business and location separation</li><li>• {selectedTemplateIds.length ? `${selectedTemplateIds.length} selected starter template${selectedTemplateIds.length === 1 ? "" : "s"}` : "A guided empty state for the first offering"}</li><li>• A single recommended next action instead of every setting at once</li></ul></div></div></Card></div>;
}
