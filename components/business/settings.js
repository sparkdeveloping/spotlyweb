...
import { ArrowRight, Bell, Building2, Eye, FileCheck2, ImagePlus, Languages, Save, UploadCloud } from "lucide-react";
...
import { businessCategories, defaultOperationalSettings } from "@/data/business-config";
import { saveBusinessProfile, uploadFile } from "@/lib/firebase-services";
import { markLaunchCriticalBusinessChange, saveBusinessOperationalSettings } from "@/lib/business-services";
...
export function SettingsView() {
  ...
  const { business, operations, branches, user, archetype, lifecycle, selectedBusinessId } = workspace;
  ...
  async function saveProfile() {
    ...
    const launchCriticalProfileChanged = profile.name.trim() !== String(business.name || "").trim()
      || profile.legalName.trim() !== String(business.legalName || "").trim()
      || profile.category !== business.category
      || profile.logo !== (business.logo || "")
      || profile.coverImage !== (business.coverImage || "");
    if (launchCriticalProfileChanged && ["in_review", "complete"].includes(lifecycle?.launchReview?.state) && !window.confirm("This changes launch-critical business identity information. Your current launch decision will need to be reviewed again. Continue?")) return;
    setSaving(true);
    ...
    try {
      await saveBusinessProfile(business.id, {
        name: profile.name.trim(), legalName: profile.legalName, category: profile.category, categories: profile.categories, description: profile.description, phone: profile.phone, whatsapp: profile.whatsapp, email: profile.email, website: profile.website, instagram: profile.instagram, facebook: profile.facebook, logo: profile.logo, coverImage: profile.coverImage, preferredLocale: profile.preferredLocale, customerNotes: profile.customerNotes
      }, user);
      if (launchCriticalProfileChanged) {
        await markLaunchCriticalBusinessChange(business.id, {
          id: "business_identity",
          label: "Business identity changed",
          description: "The trading/legal identity, primary category, or major public brand media changed after a launch decision.",
          href: businessHref("/business/settings", { businessId: selectedBusinessId, tab: "profile" })
        });
      }
      setTask({ open: true, state: "success", title: "Business profile saved", description: launchCriticalProfileChanged && lifecycle?.stage === "live" ? "The profile is saved. Your business stays live while the launch-critical change is prepared for Spotly re-review." : "The brand information is now consistent across the business workspace and public listing.", active: 4 });
    } catch (error) {
  ...
  const tabs = [
    ...
    { value: "publication", label: "Launch status" }
  ];
  ...
  return <div className="space-y-6">
    ...
    {tab === "publication" && <div className="grid gap-5 xl:grid-cols-[1fr_380px]"><ReadinessCard /><div className="space-y-5"><Card className="p-6"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-business-soft text-business"><FileCheck2 className="h-6 w-6" /></span><h2 className="mt-5 text-xl font-semibold">Final launch review</h2><p className="mt-2 text-sm leading-6 text-secondary">The authoritative launch decision now lives in the Launch Checklist. It is separate from business-access approval, settlement verification, and product-image review.</p><div className="mt-5 rounded-2xl bg-grouped p-4"><p className="text-sm font-semibold">Current business stage</p><p className="mt-1 text-sm text-secondary">{lifecycle?.statusLabel || "Preparing for launch"}</p></div><Button href={businessHref("/business/launch", { businessId: selectedBusinessId })} className="mt-5 w-full">Open launch checklist<ArrowRight className="h-4 w-4" /></Button></Card><Card className="p-5"><div className="flex items-start gap-4"><Languages className="mt-1 h-5 w-5 text-business" /><div><h3 className="font-bold">Multilingual content ready</h3><p className="mt-1 text-sm leading-6 text-secondary">English, ChiShona, and isiNdebele content can be added progressively without changing the business structure.</p></div></div></Card></div></div>}
