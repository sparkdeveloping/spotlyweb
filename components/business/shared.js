...
export function FullScreenTask({ open, state = "processing", title, description, steps = [], activeStep = 0, onDone, doneLabel = "Continue" }) {
  return <AnimatePresence>{open && <motion.div role="dialog" aria-modal="true" aria-labelledby="spotly-task-title" aria-describedby={description ? "spotly-task-description" : undefined} className="fixed inset-0 z-[120] flex items-center justify-center bg-[#071b13]/95 p-5 text-white backdrop-blur-xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
    <motion.div initial={{ opacity: 0, y: 24, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="w-full max-w-xl text-center">
      ...
      <h2 id="spotly-task-title" className="mt-8 text-3xl font-black tracking-tight">{title}</h2>
      {description && <p id="spotly-task-description" aria-live="polite" className="mx-auto mt-3 max-w-lg text-sm leading-7 text-white/70">{description}</p>}
      {steps.length > 0 && <div className="mx-auto mt-8 max-w-md space-y-2 text-left" aria-label={`Progress: ${Math.min(activeStep + 1, steps.length)} of ${steps.length}`}>{steps.map((step, index) => <motion.div key={step} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * .08 }} className={`flex items-center gap-3 rounded-2xl px-4 py-3 ${index <= activeStep ? "bg-white/10" : "bg-white/[.04] text-white/40"}`}><span className={`flex h-8 w-8 items-center justify-center rounded-xl ${index < activeStep || state === "success" ? "bg-emerald-400 text-emerald-950" : index === activeStep ? "bg-white text-emerald-950" : "bg-white/10"}`}>{index < activeStep || state === "success" ? <Check className="h-4 w-4" /> : index + 1}</span><span className="text-sm font-semibold">{step}</span></motion.div>)}</div>}
      {state !== "processing" && onDone && <Button onClick={onDone} className="mt-8 bg-white text-emerald-950 hover:bg-white/90">{doneLabel}<ArrowRight className="h-4 w-4" /></Button>}
    </motion.div>
