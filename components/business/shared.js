...
export function CompletionBanner() {
  ...
  return <div className="flex flex-col gap-4 rounded-2xl border border-business/20 bg-business-soft p-5 sm:flex-row sm:items-center"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white text-business shadow-sm"><Sparkles className="h-6 w-6" /></span><div className="min-w-0 flex-1"><p className="text-xs font-black uppercase tracking-[.14em] text-business">Recommended next step</p><h3 className="mt-1 font-bold text-emerald-950">{next?.label}</h3><p className="mt-1 text-sm leading-6 text-emerald-800">{next?.description}</p></div>{next && <Button asChild size="sm"><Link href={next.href}>Continue<ArrowRight className="h-4 w-4" /></Link></Button>}</div>;
}
