        ...
        <div className="p-5">
          <div className="flex items-start gap-3">
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 flex items-center gap-1 text-sm text-gray-500"><MapPin className="h-4 w-4" />{business.city || "Zimbabwe"}</p>
            </div>
    ...
    <Card className="flex gap-4 p-4">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-3">
          <div><h3 className="font-semibold">{product.name}</h3><p className="mt-1 line-clamp-2 text-sm text-secondary">{product.description || product.category || "Available for pickup"}</p></div>
...
function CheckoutModal({ open, onClose, business, branches, cartItems, currency, user, onComplete }) {
  ...
  useEffect(() => {
    if (open && branches.length && !form.branchId) setForm((value) => ({ ...value, branchId: branches[0].id }));
  }, [branches, form.branchId, open]);
    ...
    <Modal open={open} onClose={onClose} title="Confirm grocery pickup" size="lg">
      <form onSubmit={submit} className="space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="text-sm font-medium">Pickup branch<select {...field("branchId")} required className="input mt-2 w-full">{branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name} · {branch.city}</option>)}</select></label>
...
export function MarketplaceApp() {
  ...
  useEffect(() => {
    if (!selected?.id) { setProducts([]); setBranches([]); return undefined; }
    ...
    const stopBranches = subscribeBranches(selected.id, setBranches, () => setBranches([]));
    ...
    <div className="min-h-screen bg-[#f8f8fc] text-[#17171f]">
      <header className="sticky top-0 z-40 border-b border-gray-200/80 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex h-[72px] max-w-7xl items-center gap-4 px-4 sm:px-6">
          ...
          <div className="hidden flex-1 md:block"><SearchField value={query} onChange={setQuery} placeholder="Search groceries and businesses across Zimbabwe" /></div>
          ...
          <section>
            ...
            <div className="mt-5 overflow-hidden rounded-[28px] border border-gray-200 bg-white shadow-sm">
              <div className="grid gap-0 lg:grid-cols-[1fr_.42fr]">
                <div className="p-6 sm:p-8"><div className="flex flex-wrap gap-2"><Badge tone="purple">{selected.category}</Badge>{selected.verificationStatus === "approved" ? <Badge tone="green"><BadgeCheck className="mr-1 h-3.5 w-3.5" />Verified</Badge> : <Badge tone="neutral">Provisional · unclaimed</Badge>}</div><h1 className="mt-4 text-4xl font-bold tracking-[-.045em]">{selected.name}</h1><p className="mt-3 max-w-2xl text-base leading-7 text-gray-600">{selected.description || "This public listing was prepared to make business onboarding easier. The owner can claim it, verify details, and publish products without starting from an empty form."}</p><div className="mt-5 flex flex-wrap gap-4 text-sm text-gray-600"><span className="flex items-center gap-1"><MapPin className="h-4 w-4" />{selected.city || "Zimbabwe"}</span><span className="flex items-center gap-1"><Store className="h-4 w-4" />{branches.length || selected.branchCount || 1} branch{(branches.length || selected.branchCount || 1) === 1 ? "" : "es"}</span></div></div>
                <div className="flex flex-col justify-center border-t bg-violet-50 p-6 lg:border-l lg:border-t-0"><p className="text-sm font-semibold text-violet-900">Own or manage this business?</p><p className="mt-2 text-sm leading-6 text-violet-800">Claim the pre-filled listing, confirm each branch, and complete verification.</p><Button asChild className="mt-4"><Link href={`/claim?business=${selected.id}`}>Claim this business</Link></Button></div>
              </div>
            </div>

            <div className="mt-7 grid gap-7 lg:grid-cols-[1fr_340px]">
              <div><div className="flex items-end justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-violet-600">Grocery pickup</p><h2 className="mt-2 text-2xl font-bold">Available products</h2></div><p className="text-sm text-gray-500">{products.length} products</p></div>{products.length ? <div className="mt-5 grid gap-3 xl:grid-cols-2">{products.map((product) => <ProductCard key={product.id} product={product} quantity={cart[product.id] || 0} onChange={changeQuantity} currency={currency} />)}</div> : <EmptyState className="mt-5" icon={ShoppingBasket} title="Catalogue not published yet" description="This business is visible, but it has not published verified pickup products. Claiming and onboarding are designed to pre-fill the business details before the catalogue is added." action={<Button asChild variant="outline"><Link href={`/claim?business=${selected.id}`}>Help publish this business</Link></Button>} />}</div>
              <aside className="h-fit rounded-[24px] border border-gray-200 bg-white p-5 shadow-sm lg:sticky lg:top-24"><div className="flex items-center justify-between"><h2 className="font-bold">Your basket</h2><ShoppingBasket className="h-5 w-5 text-violet-600" /></div>{cartItems.length ? <><div className="mt-4 space-y-3">{cartItems.map((item) => <div key={item.id} className="flex items-center gap-3 text-sm"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-violet-100 font-bold text-violet-700">{item.quantity}</span><span className="min-w-0 flex-1 truncate">{item.name}</span><span className="font-semibold">{formatCurrency(Number(item.prices?.[currency] ?? item.price ?? 0) * item.quantity, currency)}</span></div>)}</div><div className="my-5 border-t border-gray-200" /><div className="flex items-center justify-between text-lg font-bold"><span>Subtotal</span><span>{formatCurrency(cartTotal, currency)}</span></div><p className="mt-2 text-xs leading-5 text-gray-500">Final availability and substitutions are confirmed by the business. Any configured service fee is calculated securely at checkout.</p><Button className="mt-5 w-full" onClick={() => setCheckout(true)}>Choose pickup time</Button></> : <div className="py-8 text-center"><ShoppingBasket className="mx-auto h-8 w-8 text-gray-300" /><p className="mt-3 text-sm font-semibold">Your basket is empty</p><p className="mt-1 text-xs leading-5 text-gray-500">Add published products to continue.</p></div>}</aside>
            </div>
          </section>
        ) : (
          <section>
      ...
      <CheckoutModal open={checkout} onClose={() => setCheckout(false)} business={selected} branches={branches} cartItems={cartItems} currency={currency} user={user} onComplete={() => setCart({})} />
    </div>
  );
}
