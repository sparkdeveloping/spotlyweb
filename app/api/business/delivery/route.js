...
const schema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("configure"), businessId: z.string().min(1), branchId: z.string().min(1), delivery: z.object({
    ...
    fulfilmentMode: z.enum(["merchant_prepared", "driver_shops"]).default("merchant_prepared"), shoppingReservePercent: z.number().min(0).max(30).optional()
  }) }),
...
export async function POST(request) {
  try {
    ...
    if (body.action === "configure") {
      ...
      await branchRef.set({
        ...
        delivery: {
          ...
          fulfilmentMode: delivery.fulfilmentMode || "merchant_prepared", shoppingReservePercent: Number(delivery.shoppingReservePercent ?? branch.delivery?.shoppingReservePercent ?? 12),
    ...
    if (body.action === "ready") {
      if (jobSnap.data().fulfilmentMode === "driver_shops") throw Object.assign(new Error("Driver-shop orders do not wait for merchant packing. Spotly dispatches the Driver after payment is confirmed."), { status: 409 });
