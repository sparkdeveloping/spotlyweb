...
export const SHARED_SIGNOUT_COOKIE = "spotly_signed_out";
...
export const SHARED_SIGNOUT_MAX_AGE_SECONDS = 365 * 24 * 60 * 60;
...
export function sharedSignoutCookieHeader(request, { clear = false } = {}) {
  const host = requestHostname(request);
  const production = isSpotlyProductionHostname(host);
  const parts = [`${SHARED_SIGNOUT_COOKIE}=${clear ? "" : "1"}`, "Path=/", "HttpOnly", "SameSite=Lax"];
  if (production) parts.push("Secure", "Domain=.spotlyafrica.com");
  parts.push(`Max-Age=${clear ? 0 : SHARED_SIGNOUT_MAX_AGE_SECONDS}`);
  if (clear) parts.push("Expires=Thu, 01 Jan 1970 00:00:00 GMT");
  return parts.join("; ");
}
