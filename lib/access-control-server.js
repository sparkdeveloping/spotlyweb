...
export async function getBusinessContext(db, user, businessId) {
  ...
  // Direct business ownership is itself a trusted access relationship. Portfolio discovery already
  // recognizes ownerIds, so selected-business APIs must not become inaccessible merely because an
  // older record predates the membership document model. ownerIds cannot be changed by merchant
  // browser writes under Firestore rules.
  const ownerIds = Array.isArray(business.ownerIds) ? business.ownerIds : [];
  if (ownerIds.includes(user.uid)) {
    return {
      business,
      membership: { id: `direct-owner:${businessId}:${user.uid}`, userId: user.uid, businessId, organizationId: business.organizationId || null, role: "business_owner", status: "active", permissions: [] },
      platformAdmin: false,
      businessWide: true,
      directOwner: true
    };
...
export async function requireSpotlyStaffPermission(db, user, required, { roles = [] } = {}) {
  if (isPlatformAdmin(user) || roles.some((role) => (user?.profile?.roles || []).includes(role)) || hasPlatformPermission(user, required)) return { platformAdmin: isPlatformAdmin(user), staffProfile: null };
  const snapshot = await db.collection("staffProfiles").doc(user.uid).get();
  if (!snapshot.exists) throw Object.assign(new Error("Spotly staff access is required."), { status: 403 });
  const profile = snapshot.data();
  if (!["active", "probation", "preboarding", "leave"].includes(profile.status)) throw Object.assign(new Error("Your Spotly staff access is not active."), { status: 403 });
  const permissions = profile.permissions || [];
  const roleAllowed = roles.includes(profile.rolePackId);
  const permitted = roleAllowed || permissions.some((item) => permissionMatches(item, required));
  if (!permitted) throw Object.assign(new Error("Your Spotly staff role does not allow this action."), { status: 403 });
  return { platformAdmin: false, staffProfile: { id: snapshot.id, ...profile } };
}
