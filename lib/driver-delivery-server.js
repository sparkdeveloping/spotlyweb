...
export async function notifyUsers(db, messaging, userIds, { title, body, href, category = "delivery" }) {
  const unique = [...new Set((userIds || []).filter(Boolean))];
  if (!unique.length) return { stored: 0, delivered: 0 };
  const batch = db.batch();
  unique.forEach((userId) => batch.set(db.collection("notifications").doc(), {
    userId, title, body, href, category, read: false, createdAt: FieldValue.serverTimestamp()
  }));
  await batch.commit();
  if (!messaging) return { stored: unique.length, delivered: 0 };
  const snapshots = await Promise.all(unique.map((userId) => db.collection("pushTokens").where("userId", "==", userId).where("active", "==", true).limit(10).get()));
  const tokens = [...new Set(snapshots.flatMap((snapshot) => snapshot.docs.map((item) => item.data().token)).filter(Boolean))];
  if (!tokens.length) return { stored: unique.length, delivered: 0 };
  try {
    const result = await messaging.sendEachForMulticast({ tokens, notification: { title, body }, data: { href, category } });
    return { stored: unique.length, delivered: result.successCount };
  } catch {
    return { stored: unique.length, delivered: 0 };
  }
}
