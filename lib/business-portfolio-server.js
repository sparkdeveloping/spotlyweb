...
export async function getBusinessPortfolio(db, user) {
  ...
  const businesses = [...businessDocs.values()].map((doc) => {
    ...
    const suspended = lifecycleSnapshot.businessState === "suspended" || String(data.status || "").toLowerCase() === "suspended";
    const attention = [];
    if (suspended) attention.push({ type: "suspension", label: "Business suspended — review status", href: lifecycleSnapshot.defaultHref });
    else if (lifecycleSnapshot.nextAction && (lifecycleSnapshot.stage !== "live" || !lifecycleSnapshot.canOperate)) attention.push({ type: lifecycleSnapshot.nextAction.id, label: lifecycleSnapshot.nextAction.label, href: lifecycleSnapshot.nextAction.href });
    if (!suspended && lifecycleSnapshot.launchReview.state === "action_required") attention.push({ type: "launch_review", label: "Spotly requested launch changes", href: lifecycleSnapshot.launchReview.href });
    ...
    return {
      ...
      lifecycleActionLabel: suspended ? "View suspension status" : lifecycleSnapshot.nextAction?.actionLabel || (lifecycleSnapshot.stage === "live" ? "Open business" : lifecycleSnapshot.stage === "review" ? "View Spotly review" : "Open launch checklist"),
  ...
  const attention = [
    ...
    ...invitations.map((item) => ({ id: `invite:${item.id}`, type: "invitation", businessId: item.businessId, businessName: item.businessName, title: `Invitation: ${item.roleLabel}`, href: `/invitations?invitation=${item.id}`, priority: 1, updatedAt: item.createdAt })),
