...
export async function getBusinessPortfolio(db, user) {
  ...
  const [branchesByBusiness, productsByBusiness, operationsByBusiness, settlement, platformSnapshot] = await Promise.all([
    recordsByBusiness(db, "branches", businessIds),
  ...
  const businesses = [...businessDocs.values()].map((doc) => {
    ...
    const enrichedBusiness = {
      id: doc.id,
      ...data,
      moneySetup: { ...(data.moneySetup || {}), ...(account ? { settlementStatus: account.status || data.moneySetup?.settlementStatus, settlementLast4: account.accountNumberLast4 || data.moneySetup?.settlementLast4 } : {}) }
    };
    ...
    const attention = [];
    if (lifecycle.nextAction && lifecycle.stage !== "live") attention.push({ type: lifecycle.nextAction.id, label: lifecycle.nextAction.label, href: lifecycle.nextAction.href });
    if (lifecycle.launchReview.state === "action_required") attention.push({ type: "launch_review", label: "Spotly requested launch changes", href: lifecycle.launchReview.href });
    if (data.status === "paused") attention.push({ type: "status", label: "Business is temporarily paused", href: lifecycle.defaultHref });
    return {
      id: doc.id,
      ...
      lifecycleLabel: lifecycle.statusLabel,
      merchantProgress: lifecycle.merchantProgress,
      defaultHref: lifecycle.defaultHref,
      lifecycleActionLabel: lifecycle.nextAction?.actionLabel || (lifecycle.stage === "live" ? "Open business" : lifecycle.stage === "review" ? "View Spotly review" : "Open launch checklist"),
      launchReviewStatus: lifecycle.launchReview.status || "",
