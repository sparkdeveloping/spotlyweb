...
export async function getBusinessPortfolio(db, user) {
  ...
  const businesses = [...businessDocs.values()].map((doc) => {
    ...
    const attention = [];
    if (lifecycleSnapshot.nextAction && lifecycleSnapshot.stage !== "live") attention.push({ type: lifecycleSnapshot.nextAction.id, label: lifecycleSnapshot.nextAction.label, href: lifecycleSnapshot.nextAction.href });
