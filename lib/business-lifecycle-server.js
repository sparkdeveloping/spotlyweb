...
export async function loadBusinessLifecycleData(db, businessId, { membership = null, userId = "" } = {}) {
  ...
  const branchResolutionPromise = loadCanonicalBusinessBranches(db, businessId, { repair: true, businessSnapshot });
  const [branchResolution, products, operationsSnapshot, settlementSnapshot, claimsSnapshot, invitationsSnapshot, directMembersSnapshot, scopedMembersSnapshot, platformSnapshot] = await Promise.all([
    branchResolutionPromise,
  ...
  const branches = branchResolution.branches;
  const rawBusiness = { id: businessSnapshot.id, ...businessSnapshot.data(), branchIds: branchResolution.branches.map((item) => item.id), branchCount: branchResolution.branches.length };
...
export function publicLifecycleSnapshot(lifecycle) {
  return {
    ...
    operationallyBlocked: Boolean(lifecycle.operationallyBlocked),
