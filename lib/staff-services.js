...
export async function saveStaffProfile(values, actor) {
  ...
  await writeAuditLog({ action: "staff.profile_saved", entityType: "staffProfiles", entityId: id, actorId: actor?.uid, actorEmail: actor?.email });
  return id;
}
...
export async function saveStaffTask(values, actor) {
  ...
  await writeAuditLog({ action: "staff.task_saved", entityType: "staffTasks", entityId: values.id, actorId: actor?.uid, actorEmail: actor?.email });
  return values.id;
}
...
export async function updateStaffTask(id, changes, actor) {
  ...
  await writeAuditLog({ action: "staff.task_updated", entityType: "staffTasks", entityId: id, metadata: changes, actorId: actor?.uid, actorEmail: actor?.email });
}
...
export async function saveStaffShift(values, actor) {
  ...
  await writeAuditLog({ action: "staff.shift_saved", entityType: "staffShifts", entityId: values.id, actorId: actor?.uid, actorEmail: actor?.email });
  return values.id;
}
...
export async function clockStaffShift(shiftId, action, actor) {
  ...
  await writeAuditLog({ action: `staff.clock_${action}`, entityType: "staffShifts", entityId: shiftId, actorId: actor?.uid, actorEmail: actor?.email });
}
...
export async function saveLeaveRequest(values, actor) {
  ...
  await writeAuditLog({ action: "staff.leave_saved", entityType: "staffLeaveRequests", entityId: values.id, actorId: actor?.uid, actorEmail: actor?.email });
  return values.id;
}
...
export async function decideLeaveRequest(id, status, reason, actor) {
  ...
  await writeAuditLog({ action: `staff.leave_${status}`, entityType: "staffLeaveRequests", entityId: id, metadata: { reason }, actorId: actor?.uid, actorEmail: actor?.email });
}
...
export async function saveWorkforceRequest(values, actor) {
  ...
  await writeAuditLog({ action: "staff.workforce_request_saved", entityType: "workforceRequests", entityId: values.id, actorId: actor?.uid, actorEmail: actor?.email });
  return values.id;
}
...
export async function saveCandidate(values, actor) {
  ...
  await writeAuditLog({ action: "staff.candidate_saved", entityType: "staffCandidates", entityId: values.id, actorId: actor?.uid, actorEmail: actor?.email });
  return values.id;
}
...
export async function saveStaffSupportRequest(values, actor) {
  ...
  await writeAuditLog({ action: "staff.support_requested", entityType: "staffSupportRequests", entityId: result.id, actorId: actor?.uid, actorEmail: actor?.email });
  return result.id;
}
