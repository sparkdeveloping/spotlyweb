...
export async function joinWaitlist(payload) {
  ...
  if (!String(payload.name || "").trim()) throw new Error("Enter your name.");
  if (!payload.consent) throw new Error("Consent is required for launch updates.");
  await setDoc(entryRef, existing.exists() ? values : { ...values, country: "ZW", status: "waiting", createdAt: serverTimestamp() }, { merge: true });
  await track(existing.exists() ? "waitlist_updated" : "waitlist_joined", { source: payload.source || "coming_soon", city: payload.city || "" });
  return { alreadyJoined: existing.exists(), id };
}
