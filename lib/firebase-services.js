...
export async function getPublicBranchesForBusiness(businessId) {
  if (!businessId) return [];
  const { db } = sdk();
  const snapshot = await getDocs(query(collection(db, "branches"), where("businessId", "==", businessId), where("public", "==", true), orderBy("name"), limit(100)));
  return snapshot.docs.map((item) => ({ id: item.id, ...item.data() }));
}
...
export function subscribePublicBusinessCatalog(businessId, callback, onError) {
  if (!businessId) return () => {};
  const { db } = sdk();
  return onSnapshot(query(collection(db, "products"), where("businessId", "==", businessId), where("published", "==", true), where("active", "==", true), orderBy("name"), limit(250)), (snapshot) => callback(snapshot.docs.map((item) => ({ id: item.id, ...item.data() }))), onError);
}
...
export function subscribePublicBranches(businessId, callback, onError) {
  if (!businessId) return () => {};
  const { db } = sdk();
  return onSnapshot(query(collection(db, "branches"), where("businessId", "==", businessId), where("public", "==", true), orderBy("name"), limit(100)), (snapshot) => callback(snapshot.docs.map((item) => ({ id: item.id, ...item.data() }))), onError);
}
...
export async function saveBranch(branch, businessId, organizationId, actor) {
  ...
  await writeClientTelemetry({ action: branch.id ? "branch.updated" : "branch.created", entityType: "branch", entityId: branchRef.id, actorId: actor?.uid, actorEmail: actor?.email, metadata: { businessId } });
  return branchRef.id;
}
