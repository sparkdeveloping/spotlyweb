...
function absoluteUrl(href = "", workspace = "customer") {
  return canonicalSpotlyUrl(href, workspace);
}

async function sendEmail(db, { to, title, body, href, workspace = "customer", eventType, userId }) {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey || !to) return { status: "not_configured" };
  const from = process.env.RESEND_FROM_EMAIL || "Spotly <onboarding@resend.dev>";
  const actionUrl = absoluteUrl(href, workspace);
...
async function pushForUser(db, messaging, userId, notification) {
  ...
  const snapshot = await db.collection("pushTokens").where("userId", "==", userId).where("active", "==", true).limit(40).get();
  const entries = snapshot.docs.map((doc) => doc.data()).filter((item) => item.token);
  const workspaceMatches = entries.filter((item) => item.workspace === notification.workspace);
  // Prefer the product origin that owns the notification. Legacy tokens created before
  // the subdomain split do not have a workspace field, so fall back to all active tokens
  // only when no correctly-scoped token exists.
  const selected = workspaceMatches.length ? workspaceMatches : entries;
  const tokens = [...new Set(selected.map((item) => item.token).filter(Boolean))];
  ...
  try {
    const result = await messaging.sendEachForMulticast({
      ...
      data: {
        href: canonicalSpotlyUrl(notification.href || "", notification.workspace || "customer"),
...
export async function notifyUser({ db, messaging = null, auth = null, userId, title, body, href = "", category = "general", workspace = "customer", module = "general", eventType = "", importance = "normal", businessId = null, entityType = null, entityId = null, email = false, push = true, forceOperationalEmail = false }) {
  ...
  const record = {
    ...
    body: safe(body, 1000),
    href: safe(canonicalSpotlyUrl(href, workspace), 600),
  ...
  const [emailResult, pushResult] = await Promise.all([
    email && operationalEmailAllowed && recipient.email
      ? sendEmail(db, { to: recipient.email, title: record.title, body: record.body, href: record.href, workspace: record.workspace, eventType: record.eventType, userId })
