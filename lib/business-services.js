...
export async function updateBusinessOrder(order, changes, actor, note = "") {
  ...
  if (["cancelled", "rejected", "void", "expired", "refunded"].includes(nextStatus)) {
    const reason = nextStatus === "rejected" ? "merchant_rejected" : nextStatus === "void" ? "admin_void" : nextStatus === "expired" ? "order_expired" : nextStatus === "refunded" ? "refund_before_fulfilment" : "customer_cancelled";
    await authenticatedFetch("/api/orders/release", { method: "POST", body: JSON.stringify({ orderId: order.id, reason, status: nextStatus }) }).catch(() => null);
  }
  if (order.customerId) {
