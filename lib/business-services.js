...
export function subscribeCatalogTemplates(callback, onError) {
  const { db } = sdk();
  return onSnapshot(
    query(collection(db, "catalogTemplates"), limit(150)),
    (snapshot) => callback(list(snapshot).sort((a, b) => String(a.name || "").localeCompare(String(b.name || ""), "en", { sensitivity: "base" })).slice(0, 100)),
    onError
  );
}
...
export function subscribeAdminTasks(callback, options = {}) {
  ...
  constraints.push(limit(Math.max(options.limit || 250, 500)));
  return onSnapshot(query(collection(db, "adminTasks"), ...constraints), (snapshot) => callback(newest(list(snapshot)).slice(0, options.limit || 250)), options.onError);
}
...
export function subscribeAllPayouts(callback, options = {}) {
  ...
  if (options.status) constraints.push(where("status", "==", options.status));
  constraints.push(limit(Math.max(options.limit || 250, 500)));
  return onSnapshot(query(collection(db, "payouts"), ...constraints), (snapshot) => callback(newest(list(snapshot)).slice(0, options.limit || 250)), options.onError);
}
