const ALL_ADMIN_SECTIONS = ["dashboard", "operations", "notifications", "organizations", "businesses", "people", "drivers", "customers", "finance", "content", "platform-map", "platform", "audit", "settings"];
...
const ROLE_SECTIONS = {
  ...
  finance_admin: ["dashboard", "organizations", "businesses", "drivers", "people", "finance", "platform-map", "audit"],
...
function hasMappedPermission(permissions = []) {
  return permissions.some((permission) => Object.keys(PERMISSION_SECTIONS).some((prefix) =>
    permission === prefix.slice(0, -1) || permission.startsWith(prefix) || permission === `${prefix}*`
  ));
}

function hasExplicitAdminGrant(profile) {
  const roles = profile?.roles || [];
  const permissions = profile?.customPermissions || [];
  return roles.some((role) => Boolean(ROLE_SECTIONS[role])) || permissions.includes("*") || hasMappedPermission(permissions);
}

export function adminSectionsForProfile(profile) {
  const roles = profile?.roles || [];
  const permissions = profile?.customPermissions || [];
  if (!hasExplicitAdminGrant(profile)) return new Set();
  if (roles.includes("super_admin") || roles.includes("admin") || permissions.includes("*")) return new Set(ALL_ADMIN_SECTIONS);
  const sections = new Set(["dashboard", "notifications"]);
...
export function hasAdminAccess(profile) {
  return hasExplicitAdminGrant(profile);
}
