...
function branchName(branch = {}) {
  return text(branch.branchName || branch.name || branch.displayName);
}

function branchIsActiveRecord(branch = {}) {
  return !["removed", "archived", "deleted"].includes(status(branch.status));
}

function branchSetupValid(branch = {}, business = {}) {
  const noPublicLocation = ["online_only", "mobile_service"].includes(business.operatingModel);
  return Boolean(
    branchIsActiveRecord(branch)
    && branchName(branch)
    && text(branch.city)
...
function branchLaunchMissing(branch = {}, business = {}) {
  const missing = [];
  if (!branch) return ["location record"];
  if (!branchName(branch)) missing.push("location name");
  if (!text(branch.address || branch.city)) missing.push("address or service area");
  if (!text(branch.phone || branch.email || business.phone || business.email || business.whatsapp)) missing.push("customer contact");
  if (!hasOpeningHours(branch) && !["online_only", "mobile_service"].includes(business.operatingModel)) missing.push("opening hours");
  return missing;
}

function branchReadinessScore(branch = {}, business = {}) {
  if (!branchIsActiveRecord(branch)) return -1000;
  let score = 0;
  if (branchSetupValid(branch, business)) score += 100;
  if (branchName(branch)) score += 10;
  if (text(branch.city)) score += 10;
  if (text(branch.address)) score += 8;
  if (text(branch.phone || branch.email || business.phone || business.email || business.whatsapp)) score += 8;
  if (hasOpeningHours(branch) || ["online_only", "mobile_service"].includes(business.operatingModel)) score += 8;
  if (branch.public !== false) score += 2;
  return score;
}

export function normalizeBusinessLifecycleInput(input = {}) {
  const business = input.business || {};
  const activeBranches = list(input.branches).filter(branchIsActiveRecord);
  const preferredId = text(
    business.primaryBranchId
    || business.primaryLocationId
    || business.defaultBranchId
    || business.branchIds?.[0]
  );
  const preferred = preferredId ? activeBranches.find((branch) => branch.id === preferredId) || null : null;
  const ranked = [...activeBranches].sort((a, b) => branchReadinessScore(b, business) - branchReadinessScore(a, business));
  const canonicalBranch = preferred && branchSetupValid(preferred, business)
    ? preferred
    : ranked.find((branch) => branchSetupValid(branch, business))
      || preferred
      || ranked[0]
      || null;
  ...
  return {
    ...
    canonicalBranch,
    canonicalBranchId: canonicalBranch?.id || ""
  };
}

export function activeSetupSteps(business = {}) {
  const archetype = businessArchetype(business);
  // Review confirms already-valid basics; it is never counted in the completion percentage.
  // Starter is a real wizard step for archetypes that include it, even when the correct choice
  // is to continue with a clean workspace.
  return SETUP_STEPS.filter((item) => archetype.setup.includes(item.id)).map((item) => ({ ...item, required: item.id !== "review" }));
}

export function evaluateSetupSteps({ business = {}, branches = [], products = [], operations = {} } = {}) {
  const normalized = normalizeBusinessLifecycleInput({ business, branches, products, operations });
  ...
  const validators = {
    ...
    location: () => branches.some((branch) => branchSetupValid(branch, business)),
    offering: () => capabilities.length > 0 || persisted.has("offering") || Boolean(completedAt),
    starter: () => persisted.has("starter") || Boolean(completedAt),
    review: () => Boolean(completedAt || persisted.has("review"))
  };

  const active = activeSetupSteps({ ...business, businessType: type });
  const baseSteps = active.map((step) => {
    const complete = step.id === "review" ? false : Boolean(validators[step.id]?.());
    return { ...step, complete, state: complete ? "complete" : "incomplete" };
  });
  const required = baseSteps.filter((step) => step.required);
  const requiredComplete = required.filter((step) => step.complete).length;
  const requiredBasicsComplete = requiredComplete === required.length;
  const structuralRequired = required.filter((step) => step.id !== "location");
  const structuralRequiredComplete = structuralRequired.filter((step) => step.complete).length;
  const structuralBasicsValid = structuralRequiredComplete === structuralRequired.length;
  const reviewPersisted = Boolean(validators.review());
  // Once the merchant has completed Review, later launch-readiness changes must not throw the
  // workspace back into first-run onboarding. Current validity is still evaluated separately and
  // can block launch through the exact affected launch check (for example Location readiness).
  const foundationEstablished = reviewPersisted || business.onboardingStatus === "complete";
  const steps = baseSteps.map((step) => step.id === "review"
    ? { ...step, complete: requiredBasicsComplete && foundationEstablished, state: requiredBasicsComplete && foundationEstablished ? "complete" : "incomplete" }
    : step);
  const percent = required.length ? Math.round((requiredComplete / required.length) * 100) : 100;
  const firstRequiredIncomplete = required.find((step) => !step.complete) || null;
  const reviewStep = steps.find((step) => step.id === "review") || null;
  const firstIncomplete = firstRequiredIncomplete || (!foundationEstablished && reviewStep ? reviewStep : null);
  ...
  return {
    ...
    requiredComplete,
    percent,
    requiredBasicsComplete,
    currentBasicsValid: requiredBasicsComplete,
    structuralBasicsValid,
    structuralRequiredTotal: structuralRequired.length,
    structuralRequiredComplete,
    foundationEstablished,
    reviewComplete: Boolean(reviewStep?.complete),
    complete: foundationEstablished && requiredBasicsComplete,
    firstIncomplete,
    firstIncompleteId: firstIncomplete?.id || null,
    canonicalBranchId: normalized.canonicalBranchId,
...
export function resolveSetupStep({ requestedStep = "", business = {}, branches = [], products = [], operations = {} } = {}) {
  ...
  // Review is the aggregate confirmation screen. An explicit Review deep link is allowed even
  // when an earlier requirement has become invalid so the merchant sees the blocker and a precise
  // Fix action instead of being silently bounced several steps backwards.
  if (requestedStep === "review" && requestedIndex >= 0) return { ...evaluation, stepId: requestedStep, redirectToLaunch: false };
...
export function getLaunchRequirements({ business = {}, archetype, operations = {}, platformSettings = {} } = {}) {
  ...
  const locationDependent = capabilities.has("pickup_orders") || capabilities.has("orders") || capabilities.has("kiosk_pickup") || capabilities.has("kiosk_checkin") || business.deliveryEnabled === true;
  return {
    profile: "required",
    // A commerce business cannot be operational without a real persisted location. Legacy
    // records sometimes carried `online_only` while still exposing pickup/kiosk capabilities;
    // treating location as optional in that case produced a false 100%/Live state.
    location: business.operatingModel === "online_only" && !locationDependent ? "optional" : "required",
...
function accessCheck({ business = {}, membership = null, claims = [], businessId = "" } = {}) {
  ...
  return {
    ...
    href: claim?.id ? `/claim/status/${claim.id}` : businessHref("/claims", { businessId }),
    required: true,
...
function locationCheck({ business = {}, branches = [], businessId = "", requirement = "required", archetype } = {}) {
  ...
  const normalized = normalizeBusinessLifecycleInput({ business, branches });
  const candidate = normalized.canonicalBranch;
  const missing = branchLaunchMissing(candidate, business).map((item) => item === "location record" ? `${noun} record` : item);
  ...
  return {
    ...
    description: !missing.length ? `${capital(noun)} details are ready for customers.` : `${candidate ? branchName(candidate) : `Main ${noun}`}: add ${missing.join(", ")}.`,
    details: { missing, branchId: candidate?.id || null },
    actionLabel: missing.length ? `Complete ${noun}` : `Review ${noun}`,
    href: businessHref(normalized.branches.length > 1 || business.operatingModel === "physical_multi" ? "/business/branches" : "/business/setup", { businessId, ...(normalized.branches.length > 1 || business.operatingModel === "physical_multi" ? {} : { step: "location" }) }),
...
export function getBusinessLifecycle(input = {}) {
  const normalized = normalizeBusinessLifecycleInput(input);
  const {
    business = {}, branches = [], products = [], operations = {}, invitations = [], members = [], claims = [], membership = null,
    selectedBusinessId = business.id || "", archetype: providedArchetype, platformSettings = {}, canonicalBranch = null
  } = normalized;
  ...
  const checks = [
    {
      ...
      state: setup.foundationEstablished && setup.structuralBasicsValid ? "complete" : "action_required",
      owner: setup.foundationEstablished && setup.structuralBasicsValid ? "none" : "merchant",
      description: !setup.foundationEstablished
        ? "Finish the foundational business setup and review it once."
        : setup.structuralBasicsValid
          ? "Business identity, operating model and foundational customer experience are confirmed."
          : "A launch-critical business-basic detail needs attention.",
      actionLabel: setup.foundationEstablished && setup.structuralBasicsValid ? "Review business details" : "Continue setup",
      href: businessHref("/business/setup", { businessId, ...(setup.firstIncompleteId ? { step: setup.firstIncompleteId } : {}) }),
      required: true,
      blocksLaunch: !(setup.foundationEstablished && setup.structuralBasicsValid),
      merchantDone: setup.foundationEstablished && setup.structuralBasicsValid
    },
    ...
    state: reviewStatus === "approved" ? "complete"
      : ["submitted", "in_review", "assigned", "re_review_submitted", "re_review_in_review"].includes(reviewStatus) ? "in_review"
        : ["changes_requested", "action_required", "rejected", "resubmission_required", "re_review_required"].includes(reviewStatus) ? "action_required"
          : access.state !== "complete" || !setup.foundationEstablished || launchBlockers.length ? "blocked" : "incomplete",
  ...
  let stage = "access";
  if (access.state === "complete") stage = setup.foundationEstablished ? "prepare" : "basics";
  ...
  if (launchReview.state === "action_required" && access.state === "complete" && setup.foundationEstablished && !["live", "paused", "suspended"].includes(businessState)) stage = "review";
  ...
  // A legacy/public flag is not enough to make a business operational. A live business that
  // loses a launch-critical invariant (especially its persisted location) remains a known
  // published record, but operational tools are gated until the invariant is repaired.
  const operationallyBlocked = ["live", "paused"].includes(businessState) && launchBlockers.length > 0;
  const canOperate = stage === "live" && businessState === "live" && !operationallyBlocked;
  const lifecycle = {
    stage,
    ...
    businessState,
    canonicalLocation: canonicalBranch ? {
      id: canonicalBranch.id || "",
      name: branchName(canonicalBranch),
      branchName: branchName(canonicalBranch),
      city: text(canonicalBranch.city),
      address: text(canonicalBranch.address),
      phone: text(canonicalBranch.phone),
      email: text(canonicalBranch.email),
      public: canonicalBranch.public !== false,
      status: status(canonicalBranch.status) || "draft",
      openingHours: canonicalBranch.openingHours || {}
    } : null,
    access,
    setup,
    requirements,
    ...
    launchBlockers,
    nextAction,
    navigationMode: businessState === "suspended" ? "suspended" : operationallyBlocked ? "prelaunch" : stage === "live" ? "live" : stage === "access" ? "access" : stage === "basics" ? "basics" : "prelaunch",
    canOperate,
    canTakeOrders: canOperate,
    canUseKiosk: canOperate,
    canUseInsights: canOperate,
    canUsePromotions: canOperate,
    canSubmitLaunchReview: access.state === "complete" && setup.foundationEstablished && launchBlockers.length === 0 && !["in_review", "complete"].includes(launchReview.state),
    canGoLive: launchReview.state === "complete" && launchBlockers.length === 0,
    operationallyBlocked,
    operationalWarnings: [
      ...(operationallyBlocked ? [{ id: "launch_invariant", label: "Business needs a launch-critical repair", description: nextAction?.label || "A required launch item is missing.", href: nextAction?.href || businessHref("/business/launch", { businessId }), owner: "merchant" }] : []),
  ...
  lifecycle.statusLabel = operationallyBlocked ? "Live · Action required" : lifecycleStatusLabel(lifecycle);
  lifecycle.defaultHref = businessState === "suspended" ? businessHref("/business/launch", { businessId }) : operationallyBlocked ? businessHref("/business/launch", { businessId }) : stage === "live" ? businessHref("/business/today", { businessId }) : businessHref("/business/launch", { businessId });
  return lifecycle;
}
