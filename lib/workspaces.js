...
export function workspaceAccess({ profile, memberships = [], staffProfile = null, driverProfile = null } = {}) {
  ...
  if (roles.has("super_admin") || profile?.customPermissions?.includes("*")) {
    ["business", "driver", "staff", "admin"].forEach((workspace) => result.add(workspace));
    return result;
  }
  ...
  if ([...roles].some((role) => ADMIN_ROLES.has(role)) || hasAdminAccess(profile) || grants.has("admin")) result.add("admin");
  return result;
}
